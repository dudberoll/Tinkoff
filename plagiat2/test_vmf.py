...
class TestVMFDistribution(TestCase):
    ...
    def test__sampling(se_lf):
        ...
        parame = distribution.join_parameters(log_probs=torch.tensor([[0.25]]).log(), means=torch.tensor([[[-2, 0]]]).float(), hidden_ik=distribution._parametrization.ipositive(
            torch.tensor([[[0.5]]])))
    ...
    def test_normalizer(se_lf):
        ...
        for (dim, k) in itertools.product(dims, ['separate', 'norm', 1, 2]):
            with torch.no_grad():
                ...
                hidden_ik_gt = torch.randn(batch_sizes, 1, 1) if not isinstance(k, Number) else distribution._parametrization.ipositive(
                    torch.full((batch_sizes, 1, 1), 1 / float(k)))
