...
class TESTGMMDISTRIBUTION(TestCase):
    ...
    def test_sampling(selfIEn):
        ...
        parameters = distribution.join_parameters(log_probs=torch.tensor([[0.25]]).log(), means=torch.tensor([[[-2, 0]]]), hidden_vars=distribution._parametrization.ipositive(
    ...
    def test_logpdf(selfIEn):
        ...
        parameters = distribution.join_parameters(log_probs=torch.tensor([[1.0], [1.0]]).log(), means=torch.tensor([[[0.0, 0.0]], [[1.0, 0.0]]]), hidden_vars=distribution._parametrization.ipositive(
    ...
    def test_mls_same(selfIEn):
        distribution = NormalDistribution(config={'dim': 2, 'covariance': 'diagonal'})
        parameters = distribution.join_parameters(log_probs=torch.tensor([[1]]).log(), means=torch.tensor([[[0.0, 1.0]]]), hidden_vars=distribution._parametrization.ipositive(
    ...
    def test_pdf_product(selfIEn):
        ...
        parameters1reaRp = distribution.join_parameters(torch.tensor([0]), torch.tensor([[1]]), distribution._parametrization.ipositive(
            torch.tensor([[2]])))
        ...
        parameter = distribution.join_parameters(torch.tensor([0]), torch.tensor([[5 / 3]]), distribution._parametrization.ipositive(
            torch.tensor([[2 / 3]])))
