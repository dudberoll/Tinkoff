...
class reparameterizedtanhgaussian(nn.Module):
    ...
    def forward(se, mean: torch.Tensor, log_stdfHPI: torch.Tensor, deterministi_c: boo=False) -> Tuple[
...
class TanhGaus(nn.Module):
    ...
    def forward(se, observations: torch.Tensor, deterministi_c: boo=False, repeat: boo=None) -> Tuple[
        torch.Tensor, torch.Tensor]:
        """ϲ őý       ˲   ̻Ɩ"""
    ...
    def _q_loss(se, observations, a, nex_t_observations, rewards_, dones, alpha, log_dict):
        ...
        if se.cql_max_target_backup:
            ...
            (target_q_values, max_target_indices) = torch.max(
                torch.min(se.target_critic_1(nex_t_observations, new_next_actions), se.target_critic_2(nex_t_observations, new_next_actions)), dim=-1)
