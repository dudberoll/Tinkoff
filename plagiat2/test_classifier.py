...
class TestVMFClassifierxNF(TestCase):
        ...
        def test_infer(self):
                ...
                with torch.no_grad():
                        c.weight.copy_(distributi.join_parameters(log_probs=torch.zeros(n, 1), means=cls_means, hidden_ik=distributi._parametrization.ipositive(
                            torch.full((n, 1, 1), 1e-10))))
                        c.log_scale.copy_(torch.zeros([]) + 0)
                me = torch.nn.functional.normalize(torch.randn(b, 1, dimvTFbt), dim=-1)
                parametersv = distributi.join_parameters(log_probs=torch.zeros(b, 1), means=me, hidden_ik=distributi._parametrization.ipositive(
