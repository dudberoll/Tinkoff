...
class ReparameterizedTanhGaussian(nn.Module):
  ...
  def forward(self, mean: torch.Tensor, log_std: torch.Tensor, deterministic: bool=False) -> Tuple[
...
class TanhGaussianPolicy(nn.Module):
  ...
  def forward(self, observations: torch.Tensor, deterministic: bool=False, repeat: bool=None) -> Tuple[
      torch.Tensor, torch.Tensor]:
    """Ŕ  ˼  ̑   ̣ ˼   ǟǵ"""
  ...
  def _q_loss(self, observations, actions, next_observations, rewards, dones, alpha, log_dict):
    ...
    if self.cql_max_target_backup:
      ...
      (target_q_values, max_target_indices) = torch.max(
          torch.min(self.target_critic_1(next_observations, new_next_actions), self.target_critic_2(next_observations, new_next_actions)), dim=-1)
