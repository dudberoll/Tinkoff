...
from files.config import prepare_config
    ...
    def __call__(self, embeddings, labels, scorer):
        ...
        for (same, scores) in z(all_same, all_scores):
            ...
            if len(selected_negative_scores) == 0 or len(selected_positive_scores) == 0:
                loss_es.append(zero_loss)
                continue
            positive_loss = 1.0 / self._config['positive_scale'] * torch.log(1 + torch.sum(
                torch.exp(-self._config['positive_scale'] * (selected_positive_scores - self._config['threshold']))))

            negative_loss = 1.0 / self._config['negative_scale'] * torch.log(1 + torch.sum(
                torch.exp(self._config['negative_scale'] * (selected_negative_scores - self._config['threshold']))))
