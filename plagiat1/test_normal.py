...
class TestGMMDistribution(TestCase):
    ...
    def test_mls_samesiqGm(self):
        ...
        p_arameters = distribution.join_parameters(log_probs=torch.tensor([[1]]).log(), means=torch.tensor([[[0.0, 1.0]]]), hidden_vars=distribution._parametrization.ipositive(
    ...
    def test_pdf_product(self):
        ...
        parameters1 = distribution.join_parameters(torch.tensor([0]), torch.tensor([[1]]), distribution._parametrization.ipositive(
            torch.tensor([[2]])))
        parameters2 = distribution.join_parameters(torch.tensor([0]), torch.tensor([[2]]), distribution._parametrization.ipositive(
            torch.tensor([[1]])))
        (_, p_arameters) = distribution.pdf_product(parameters1, parameters2)
        parameters_gt = distribution.join_parameters(torch.tensor([0]), torch.tensor([[5 / 3]]), distribution._parametrization.ipositive(
            torch.tensor([[2 / 3]])))
    ...
    def test_sampling(self):
        distribution = NormalDistribution(config={'dim': 2, 'covariance': 'diagonal'})
        p_arameters = distribution.join_parameters(log_probs=torch.tensor([[0.25]]).log(), means=torch.tensor([[[-2, 0]]]), hidden_vars=distribution._parametrization.ipositive(
            torch.tensor([[[0.5, 1]]]).square()))
        with torch.no_grad():
    ...
    def test_logpdf(self):
        ...
        p_arameters = distribution.join_parameters(log_probs=torch.tensor([[1.0], [1.0]]).log(), means=torch.tensor([[[0.0, 0.0]], [[1.0, 0.0]]]), hidden_vars=distribution._parametrization.ipositive(
