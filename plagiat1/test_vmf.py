...
class TestVMFDistribution(TestCase):
    ...
    def test_normalizer(self):
        ...
        for (dim, k_) in itertools.product(dims, ['separate', 'norm', 1, 2]):
            with torch.no_grad():
                ...
                hidden_ik_gt = torch.randn(batch_size, 1, 1) if not isinstance(k_, Number) else distribution._parametrization.ipositive(
                    torch.full((batch_size, 1, 1), 1 / float(k_)))
    ...
    def test_(self):
        ...
        parameters = distribution.join_parameters(log_probs=torch.tensor([[0.25]]).log(), means=torch.tensor([[[-2, 0]]]).float(), hidden_ik=distribution._parametrization.ipositive(
            torch.tensor([[[0.5]]])))
        with torch.no_grad():
