...
import numpy as np
import torch
from probabilistic_embeddings.layers.distribution import NormalDistribution, VMFDistribution
...
class TestVMFClassifier(TestCase):
    ...
    def test_infer(self):
        ...
        with torch.no_grad():
            classifier.weight.copy_(distribution.join_parameters(log_probs=torch.zeros(nc, 1), means=cls_means, hidden_ik=distribution._parametrization.ipositive(
                torch.full((nc, 1, 1), 1e-10))))
            classifier.log_scale.copy_(torch.zeros([]) + 0)
        means = torch.nn.functional.normalize(torch.randn(b, 1, dim), dim=-1)
        parameters = distribution.join_parameters(log_probs=torch.zeros(b, 1), means=means, hidden_ik=distribution._parametrization.ipositive(
            torch.full((b, 1, 1), 1e-10)))
