...
class VMFClassifier(torch.nn.Module):
    ...
    def forward(self, PARAMETERS, labels=None, scorer=None):
        ...
        if labels is not None and (not self._config['deterministic_target']):
            sampl_e_parameters = self._distribution.join_parameters(log_probs=torch.zeros(B, k, 1, dtype=dtype, device=device), means=sample.unsqueeze(-2), hidden_ik=self._distribution._parametrization.ipositive(
                torch.ones(B, k, 1, 1, dtype=dtype, device=device) / sc))
